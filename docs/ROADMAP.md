...
- [ ] 웹훅 핸들러 크레딧 충전 로직으로 수정
- [ ] 모바일 결제 완료 페이지 크레딧 기준으로 수정
...
> 현재는 월 ₩5,000 구독제로 운영 중. 향후 크레딧 기반 결제로 전환하여 사용한 만큼만 과금하는 모델로 변경.

- [ ] 크레딧 단위 및 가격 정책 설계
- [ ] 크레딧 잔액 관리 (DB 스키마, 충전/차감 로직)
- [ ] 크레딧 충전 결제 플로우 (PortOne 연동)
- [ ] 게임 열람/다운로드 시 크레딧 차감 로직
- [ ] 크레딧 잔액 UI (헤더, 마이페이지)
- [ ] 기존 구독자 마이그레이션 방안
